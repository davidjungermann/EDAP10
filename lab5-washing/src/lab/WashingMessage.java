...
/**
 * Class used for messaging - from washing programs to spin controller
 * (SPIN_xxx) - from washing programs to temperature controller (TEMP_xxx) -
 * from washing programs to water controller (WATER_xxx) - from controllers to
 * washing programs (ACKNOWLEDGMENT)
 */
public class WashingMessage {
  ...
  private final MessagingThread<WashingMessage> sender;
   ...
   * @param value   an optional value, such as a water level for WATER_FILL, or a
   *                temperature for TEMP_SET
   */
  public WashingMessage(MessagingThread<WashingMessage> sender, int command, double value) {
    this.sender = sender;
  ...
  public WashingMessage(MessagingThread<WashingMessage> sender, int command) {
  ...
  public MessagingThread<WashingMessage> getSender() {
    return sender;
  }
